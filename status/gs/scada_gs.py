...
#parameter
spreadsheetname = "Scada"
sheet = ['scada', 'frtu', 'substation']
sheet1 = ['event', 'map']
...
#credentials = service_account.Credentials.from_service_account_file('credentials.json', scope)
credentials = service_account.Credentials.from_service_account_info(st.secrets['gcp_service_account'], scopes = scope)

client = Client(scope=scope,creds=credentials)
#client = gspread.authorize(credentials)
url = "https://docs.google.com/spreadsheets/d/1gxgql4UhJBEcM3eayV22R57mI4i4ZdvTGmwhcvIxdiY/edit?usp=sharing"
...
#@st.cache_data
def homepage(fn):
    df = conn.read(spreadsheet=url, worksheet=fn, ttl="10m")
    return df
...
def main():
    ...
    scada_data = homepage(sheet[0]) #scada
    rtu_data = homepage(sheet[1]) #frtu

    df = pd.merge(scada_data, rtu_data, how="outer", on=['Name'], left_index=False, right_index=False)
    st.write(df)
...
#option_funct == สถานะอุปกรณ์
    if funct_select == option_funct[0]:
        ...
        dev_state = st.sidebar.multiselect('เลือก State',options=scada_data['State'].unique().tolist(),default=scada_data['State'].unique().tolist())
        ...
        df_selection  = df.query('`ประเภทอุปกรณ์` == @dev_group & `จุดติดตั้ง` == @dev_install & `State` == @dev_state & `การไฟฟ้า` == @dev_utility & `Master` == @dev_master')
        ...
        with col1:
            df_sub= df[df['จุดติดตั้ง'] == 'สถานีไฟฟ้า']
        ...
        with col2:
            df_feed = df_selection[df_selection['จุดติดตั้ง'] == 'ระบบจำหน่าย']
        ...
        with col3:
            df_tran = df_selection[df_selection['จุดติดตั้ง'] == 'ระบบสายส่ง']
        ...
        with col4:
            df_vspp1 = df_selection[df_selection['จุดติดตั้ง'] == 'VSPP(ผชฟ)']
            df_vspp2 = df_selection[df_selection['จุดติดตั้ง'] == 'VSPP']
            df_spp = df_selection[df_selection['จุดติดตั้ง'] == 'SPP(ผชฟ)']
